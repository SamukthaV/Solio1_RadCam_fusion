# Testing Navigation using GNSS data            (Development Code)
...
def navigation_output(latitude, longitude, Current_Bearing):
    ...
    if (distance_to_final_waypoint > 1 and wp < wp_len):  # to check if the final point is not less than 1m
        ...
        #     const_speed = pothole_speed
        # elif (
        #     saw_pothole == 1 and pothole_flag == 0
        ...
        #         const_speed = speed
        #         print(
        #             "                                               LOOK_AHEAD_DISTANCE     False Detection of Speedbump"
        #         )

        # print(f"GNSS Z-Acceleration : {acc_z:.1f} | {hit_time}")

        # if (
        #     pothole_flag == 1
        # ):  # Pothole just crossed from the vision but will the vehicle
        #     saw_pothole = pothole_flag

        # if pothole_flag == 1 or saw_pothole == 1:
        #     log_and_print(
        #         f"SpeedBump Detected via present:{pothole_flag}  past:{saw_pothole}"
        #     )
